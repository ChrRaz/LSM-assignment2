...
int main(int argc, char *argv[]) {
  ...
  if (world_rank == 0) {
    printf("mpi3: %s (%3dx%3d) %d %lf %.1lf %d", argv[0], H - 2, W - 2, iter_max, tolerance, start_T, output_type);
    ...
    case 4:
      ...
      for (int i = 1; i < world_size; i++) {
        MPI_Recv(u+W*W, H/world_size*W*W, MPI_DOUBLE, i, 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        write_vtk(vtk, H/world_size, W, u+W*W);
      }
      fclose(vtk);
