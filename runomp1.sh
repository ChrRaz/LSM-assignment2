#!/bin/bash
#BSUB -J poissonomp
#BSUB -o logs/%J_omp1.out.txt
#BSUB -e logs/%J_omp1.err.txt
#BSUB -q hpc
#BSUB -n 24
...
do
    ...
    do
        ...
        do
            ...
            mpirun --map-by ppr:4:node --bind-to socket --rank-by socket -x "OMP_NUM_THREADS=${N[$n]}" ./poisson "${H[$h]}" "${W[$w]}" $iter_max $tolerance $start_T $output_type
        done
    done
done
